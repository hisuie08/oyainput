...
{
	char cmd[] = GREP_CMD " -E 'Name=|Handlers|EV=' /proc/bus/input/devices | " GREP_CMD " -B2 -- 'EV=1[02]001[3Ff]' | " GREP_CMD " -v '^--$'";
	...
	*devcnt = 0;
	...
	do
	{
		...
		if (!fgets(buff1, BUFSIZE, pipe))
			break;
		if (!fgets(buff2, BUFSIZE, pipe))
			break;
		if (!fgets(buff3, BUFSIZE, pipe))
			break;
